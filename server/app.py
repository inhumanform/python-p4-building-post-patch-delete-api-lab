...
def baked_goods():
    if request.method == 'GET':
        baked_goods = []
        for n in BakedGood.query.all():
            baked_goods.append(n.to_dict())
    ...
    # def baked_goods():
    # if request.method == 'GET':
    #     baked_goods = [n.to_dict() for n in BakedGood.query.all()]
    #     return make_response(baked_goods, 200)
...
def baked_goods_by_id(id):
    ...
    if request.method == 'GET':
        ...
        db.session.commit()
        return make_response({'message': 'Pastry Eradicated'}, 200 )
